...
**/
void lbm_comm_sync_ghosts_horizontal( lbm_comm_t * mesh, Mesh *mesh_to_process, lbm_comm_type_t comm_type, int target_rank, int x ,int tag)
{
	//vars
	MPI_Status status;
 ...
 * @param mesh_to_process Mesh a utiliser lors de l'échange des mailles fantomes
**/
void lbm_comm_sync_ghosts_diagonal(Mesh *mesh_to_process, lbm_comm_type_t comm_type, int target_rank, int x ,int y,int tag)
{
	//vars
	MPI_Status status;
	...
	{
		case COMM_SEND:
			MPI_Send( Mesh_get_cell( mesh_to_process, x, y ), DIRECTIONS, MPI_DOUBLE, target_rank, tag, MPI_COMM_WORLD);
			break;
		case COMM_RECV:
			MPI_Recv( Mesh_get_cell( mesh_to_process, x, y ), DIRECTIONS, MPI_DOUBLE, target_rank, tag, MPI_COMM_WORLD, &status);
			break;
		default:
			fatal("Unknown type of communication.");
	}
 ...
 * @param mesh_to_process Mesh a utiliser lors de l'échange des mailles fantomes
**/
void lbm_comm_sync_ghosts_vertical(Mesh *mesh_to_process, lbm_comm_type_t comm_type, int target_rank, int y,int tag)
{
	//vars
	MPI_Status status;
	unsigned x;
...
void lbm_comm_ghost_exchange(lbm_comm_t * mesh, Mesh *mesh_to_process )
{
	...
	//top left
	//lbm_comm_sync_ghosts_diagonal(mesh_to_process,COMM_SEND,mesh->corner_id[CORNER_TOP_LEFT],1,1,4);
	//lbm_comm_sync_ghosts_diagonal(mesh_to_process,COMM_RECV,mesh->corner_id[CORNER_BOTTOM_RIGHT],mesh->width - 1,mesh->height - 1,4);

	//prevend comm mixing to avoid bugs
	//MPI_Barrier(MPI_COMM_WORLD);

	//bottom left
	//lbm_comm_sync_ghosts_diagonal(mesh_to_process,COMM_SEND,mesh->corner_id[CORNER_BOTTOM_LEFT],1,mesh->height - 2,5);
	//lbm_comm_sync_ghosts_diagonal(mesh_to_process,COMM_RECV,mesh->corner_id[CORNER_TOP_RIGHT],mesh->width - 1,0,5);

	//prevend comm mixing to avoid bugs
	//MPI_Barrier(MPI_COMM_WORLD);

	//top right
	//lbm_comm_sync_ghosts_diagonal(mesh_to_process,COMM_SEND,mesh->corner_id[CORNER_TOP_RIGHT],mesh->width - 2,1,6);
	//lbm_comm_sync_ghosts_diagonal(mesh_to_process,COMM_RECV,mesh->corner_id[CORNER_BOTTOM_LEFT],0,mesh->height - 1,6);
	...
	//bottom right
	//lbm_comm_sync_ghosts_diagonal(mesh_to_process,COMM_SEND,mesh->corner_id[CORNER_BOTTOM_RIGHT],mesh->width - 2,mesh->height - 2,8);
	//lbm_comm_sync_ghosts_diagonal(mesh_to_process,COMM_RECV,mesh->corner_id[CORNER_TOP_LEFT],0,0,8);
...
void save_frame_all_domain( MPI_File fp, Mesh *source_mesh)
{
	...
	// 	save_frame(fp,source_mesh);
	// }
	MPI_Request req = save_frame(fp,source_mesh,0);
